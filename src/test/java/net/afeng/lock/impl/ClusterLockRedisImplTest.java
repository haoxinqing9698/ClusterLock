...
public class ClusterLockRedisImplTest {
	@Autowired
	@Qualifier("clusterLockRedisImpl")
	private ClusterLock clusterLock;

	private static final Logger log = LogManager.getLogger(ClusterLockRedisImplTest.class);
	//共享的锁状态
	...
	public void ClusterLockTest() throws ExecutionException{
		...
		Future<Boolean> f4 =  service.submit(
				new Callable<Boolean>() {
					public Boolean call() {
					//获取到锁设置30秒的等待时间，20秒的过期时间；
					//由于上面的线程设置了20秒的过期，因此本次获取锁需要等到锁内容过期才能获取到
					long t1 = System.currentTimeMillis();
					boolean lockresult = clusterLock.lock(lockString, 30000L, 20);
					long t2 = System.currentTimeMillis();
					System.out.println("获取到锁消耗的时间："+(t2-t1));
					Assert.assertTrue(lockresult);
					Assert.assertTrue(t2-t1>15000L);
					return lockresult;
				};
			}
		);
